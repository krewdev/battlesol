...
const PresaleProgress: React.FC = () => {
    const raised = 356.78; // Mocked data
    const hardCap = 1000;
...
const LandingPage: React.FC<LandingPageProps> = ({ onConnectWallet }) => {
    const presaleRate = 5000; // 5000 $SHIP per 1 SOL
        ...
        <div className="flex flex-col items-center justify-center min-h-screen text-center p-4 animate-fade-in">
            <header className="absolute top-0 left-0 w-full p-6 flex justify-between items-center">
                 ...
                 <button
                    ...
                    className="bg-cyan-glow/80 text-navy-900 font-bold py-2 px-4 rounded-lg transition-all duration-300 hover:bg-white hover:shadow-cyan text-sm"
                >
                    Connect & Enter App
                </button>
            </header>
            ...
            <div className="bg-navy-800/50 backdrop-blur-md border border-navy-700 rounded-2xl p-8 md:p-12 shadow-2xl max-w-2xl w-full">
                ...
                <div className="bg-navy-900/70 p-6 rounded-lg border border-navy-700">
                    ...
                    <button
                      onClick={onConnectWallet}
                      className="mt-6 w-full bg-magenta-glow text-white font-bold font-orbitron py-4 px-8 rounded-lg uppercase tracking-wider transition-all duration-300 hover:bg-magenta-glow/80 hover:shadow-magenta transform hover:scale-105"
                    >
